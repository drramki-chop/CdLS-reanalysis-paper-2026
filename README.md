...
The processed/derived datasets generated and analyzed in this study are
deposited at **FigShare** under DOI
[10.6084/m9.figshare.32595099](https://doi.org/10.6084/m9.figshare.32595099)
("*CdLS Reanalysis – Rajagopalan R et al. 2026*"). The deposit contains the
following files:

| File | Data level | Description |
|---|---|---|
| `CdLS_renalaysis_cohort_sample_table_Rajagopalan_et_al_2026.xlsx` | Sample-level metadata | Cohort/sample table listing every sample ID (e.g., `CDL-XXX-##X_R#`) included in the reanalysis, together with associated demographic and sequencing metadata. This table is the authoritative reference for matching sample IDs to the result files below. |
| `cdls_rnaseq_counts.txt` | Processed sequencing data | Gene-level raw RNA-seq read-count matrix (rows = Ensembl gene IDs/gene symbols; columns = sample IDs) — the direct input to the OUTRIDER expression-outlier pipeline (`outrider.R`). |
| `cdls_outrider_omim_annotated.csv` | Analysis-derived results | Per-sample, per-gene OUTRIDER aberrant expression-outlier statistics (p-value, adjusted p-value, z-score, log2 fold-change, normalized counts, etc.), annotated with OMIM phenotype/inheritance/disease-association information. |
| `leafcutter_outlier_clusterPvals.txt.gz` | Analysis-derived results | LeafCutterMD per-sample, per-intron-cluster splicing-outlier p-values. |
| `leafcutter_outlier_effSize.txt.gz` | Analysis-derived results | LeafCutterMD per-sample, per-intron-cluster splicing-outlier effect sizes, paired one-to-one with the p-value file above. |
| `CdLS_X01_GMKF_cnvpytor.vcf.files.tar.gz` | Analysis-derived results | Per-sample CNVpytor CNV-calling output VCFs (see `combined_scripts_for_paper.sh`), packaged as a single archive. |

**Processed sequence variant files.** Processed small-variant, CNV, and SV
call sets generated by DRAGEN and prioritized by Exomiser as part of the
Gabriella Miller Kids First (GMKF) project are accessible through the GMKF
Data Resource Center (DRC) portal at
[https://portal.kidsfirstdrc.org](https://portal.kidsfirstdrc.org) (subject to
the DRC's data-access procedures). CNVpytor CNV-calling outputs generated for
this reanalysis are additionally provided directly in the FigShare deposit
above (`CdLS_X01_GMKF_cnvpytor.vcf.files.tar.gz`).

**Sample IDs.** All sample identifiers used throughout the deposited result
files (e.g., `CDL-207-04P_R1`, `CDL-592P-R`) follow the cohort's internal
`CDL-###-##[P/M/F][_R#]` naming convention (proband/mother/father, with an
RNA-seq replicate suffix where applicable) and are listed in full — together
with demographic and family-relationship annotations — in
`CdLS_renalaysis_cohort_sample_table_Rajagopalan_et_al_2026.xlsx`.
